...
Use `train_ar.py` as the main entry point/templace:

```bash
uv run scripts/train_ar.py --config config/base.yml --device 'cuda:0' --epochs 20 --model '{model_name}' --seq_length 16 --batch_size 4 --tracker wandb --name '{run_name}'
```
