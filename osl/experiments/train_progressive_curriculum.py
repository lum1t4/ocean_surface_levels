"""
Direction 1b: Progressive training with curriculum learning.

Start training on easy (short horizon) predictions and progressively expand
to hard (long horizon). The curriculum linearly ramps max_t from 2 to T over
a configurable fraction of total epochs.

# Example run (test)
uv run osl/experiments/pixel/train_progressive_curriculum.py \
    --config config/pixel/base.yml config/pixel/progressive.yml \
    --epochs 1 --device cuda:0 --batch_size 4 --name "curriculum_test" \
...
uv run osl/experiments/pixel/train_progressive_curriculum.py \
    --config config/pixel/base.yml config/pixel/progressive.yml \
    --epochs 100 --device cuda:0 --batch_size 4 --name "progressive_curriculum_unet_008" \
    --model unet-time --seq_length 8 --variables sla --normalize \
    --prediction_type sample --curriculum_warmup 0.5 --tracker wandb
"""
...
def curriculum_max_t(epoch: int, total_epochs: int, T: int, warmup_frac: float = 0.5) -> int:
    """Linearly increase max timestep from 2 to T over warmup_frac of training."""
    warmup_epochs = int(total_epochs * warmup_frac)
    if epoch >= warmup_epochs:
        return T
    progress = epoch / max(warmup_epochs, 1)
    return max(2, int(2 + progress * (T - 2)))
...
def model_train_step(ctx: TrainContext, batch_idx: int, batch: dict) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    # Curriculum: sample timesteps up to the current curriculum max_t
    max_t = getattr(ctx, '_curriculum_max_t', T)
    ts = torch.randint(1, max_t, (B,))
...
def schedule_train_epoch(ctx: TrainContext):
    ...
    # Curriculum: compute max_t for this epoch
    T = ctx.config.seq_length
    warmup_frac = getattr(ctx.config, 'curriculum_warmup', 0.5)
    ctx._curriculum_max_t = curriculum_max_t(ctx.curr_iter, ctx.config.epochs, T, warmup_frac)
    LOGGER.info(f"Curriculum: max_t = {ctx._curriculum_max_t} / {T}")
    ...
    ctx.metrics["train/curriculum_max_t"] = ctx._curriculum_max_t
...
def main(config: IterableSimpleNamespace):
    ...
    LOGGER.info(f"Curriculum warmup: {getattr(config, 'curriculum_warmup', 0.5)}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Progressive training with curriculum learning")
    ...
    # Curriculum specific
    parser.add_argument("--curriculum_warmup", type=float, help="Fraction of epochs for curriculum ramp-up (default: 0.5)")
